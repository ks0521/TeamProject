...
public class DropTableSO : ScriptableObject
{
    ...
    public List<DropedItem> GetDroppedItems(float dropRate)
    {
        ...
        foreach (var items in dropList)
        {
            ...
            if (value < Mathf.Clamp01(items.chance * (1 + dropRate))) //확률 뽑아서 당첨이면 드롭되는 아이템 리스트에 추가
            {
                int amount;
                if (items.minAmount > items.maxAmount)
                    amount = 1;
                else
                    amount = Random.Range(items.minAmount, items.maxAmount + 1);
